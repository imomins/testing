<%@ WebHandler Language="C#" Class="PendingStudentHandler" %>
...
using System.IO;
using  System.Text;
public class PendingStudentHandler : IHttpHandler, IReadOnlySessionState
{
    ...
    public void ProcessRequest (HttpContext context) 
    {
        ...
        else
        {
            ...
            //Collection<Student> student = GetAllStudent();
    ...
    public bool CheckBannerID(string bannerID)
    {
        bool result = true;
    ...
    private void UpdateStudent(int StudentOID, string BannerStudentName, string BannerStudentIDNumber, string TermCodeofProgramEnrollment, string FullTimeOrPartTimeIndicator, double CumulativeGPA, double CreditsAttempted, double CreditsEarned, string LatestCompassPrealgebraTestScore, string LatestCompassAlgebraTestScore, string LatestCompassWritingTestScore, string LatestCompassReadingTestScore, string LatestACTEnglishAssessmentScore, string LatestACTMathAssessmentScore, string LatestACTReadingAssessmentScore, string LatestACTScienceAssessmentScore, DateTime LatestTestingDate, string HighSchoolName, DateTime HighSchoolGraduationDate, string HomeTelephoneNumber, string MailingAddressLineOne, string MailingAddressLineTwo, string MailingAddressLineThree, string City, string StateName, string ZipCode, string EmailAddress, DateTime ImportDateFileCreationDate, string PreprogramIndicator, string MajorProgramEnrollmentName,string Status)
    {
        ...
        
        {
            ...
            std.LatestTestingDate = DateTime.Now;
            ...
            std.State=StateName;
            std.ZIPCode=ZipCode;
            std.ImportDate=DateTime .Now;
            std.EmailAddress=EmailAddress;
            //std.PreprogramIndicator=PreprogramIndicator;
            std.PreprogramIndicator = "X";
            std.MajorProgramEnrollment = MajorProgramEnrollmentName;
            //std.Status = "Approved";
            std.Status = Status ;
            Result = std.UpdatePendingStudent(std);
            if (Result==true)
            {
                Answer answer = new Answer();
                answer.UpdateAnswer(std.StudentOID,std.StudentID );
                answer.UpdateScoreDetailTable(std.StudentOID, std.StudentID);
            }
    ...
    //private Collection<Student> GetAllStudent()
    {
        ...
        using (OdbcConnection connection = new OdbcConnection(connectionString))
        {
            using (OdbcCommand command = new OdbcCommand())
            {
                ...
                command.Connection = connection;
                command.CommandText = "{CALL PendingStudent_SelectjqGrid(?,?,?,?,?)}";
                //command.CommandText = "{CALL PendingStudent_SelectjqGrid}";
                command.CommandType = CommandType.StoredProcedure;
       ...
       // AE = (AE == null) ? "" : AE;
        ...
        using (OdbcConnection connection = new OdbcConnection(connectionString))
        {
            using (OdbcCommand command = new OdbcCommand())
            {
                ...
                command.CommandText = "{CALL PendingStudent_Search(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
                command.CommandType = CommandType.StoredProcedure;
    ...
    private string BuildJQGridResults(Collection<Student> students, int numberOfRows, int pageIndex, int totalRecords)
    {
        ...
        foreach (Student student in students)
        {
            ...
            row.cell[31] = student.Status;
            string url="Update.aspx?StudentOID="+student .StudentOID+"<a href ='../../images/edit-icon.png'>Edit</a>";
            //row.cell[31] = "<a  href=" + url + ">Edit</a>";                
            row.cell[32] = "Edit";                
            rows.Add(row);
        }
