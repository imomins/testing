...
public partial class pg_import_pell : System.Web.UI.Page
{
    ...
    public void SaveToDB(string path, string fileName)
    {
        ...
        foreach (DataRow dataReader in dtAlert.Rows)
        {
            try
            {
                ...
                string PELL = "PELL";
                string insertQuery = "insert into NTO(BannerID,Type) values('" + bannerID + "','" + PELL + "')";
    ...
    protected void LinkButtonNTO_Click(object sender, EventArgs e)
    {
        ...
        Response.AppendHeader("Content-Disposition", "attachment; filename=PELL.CSV");
        Response.TransmitFile(Server.MapPath("~/file/PELL.CSV"));
        Response.End();
    }
