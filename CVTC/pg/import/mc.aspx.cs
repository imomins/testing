...
public partial class pg_import_mc : System.Web.UI.Page
{
    ...
    protected void LinkButtonNTO_Click(object sender, EventArgs e)
    {
        ...
        Response.AppendHeader("Content-Disposition", "attachment; filename=MC.CSV");
        Response.TransmitFile(Server.MapPath("~/file/MC.CSV"));
        Response.End();
    }
    ...
    public void SaveToDB(string path, string fileName)
    {
        ...
        foreach (DataRow dataReader in dtAlert.Rows)
        {
            try
            {
                ...
                string MC = "MC";
                string insertQuery = "insert into NTO(BannerID,Type) values('" + bannerID + "','"+MC+"')";
