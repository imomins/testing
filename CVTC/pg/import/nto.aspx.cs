...
public partial class pg_import_nto : System.Web.UI.Page
{
    ...
    public void SaveToDB(string path, string fileName)
    {
        ...
        foreach (DataRow dataReader in dtAlert.Rows)
        {
            try
            {
               ...
               // string insertQuery = "insert into NTO(BannerID,Type) values('" + bannerID + "','" + type + "')";
                string nto = "NTO";
                string insertQuery = "insert into NTO(BannerID,Type) values('" + bannerID + "','" + nto + "')";
    ...
    {
        Response.AppendHeader("Content-Disposition", "attachment; filename=NTO.CSV");
        Response.TransmitFile(Server.MapPath("~/file/NTO.CSV"));
        Response.End();
    }
