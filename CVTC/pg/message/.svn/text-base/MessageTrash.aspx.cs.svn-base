...
public partial class pg_message_MessageTrash : System.Web.UI.Page
{
    ...
    private void PopulateMessageBox()
    {
        try
        {
            ...
            GridViewMessageBox.DataSource = mess.GetMessageTrashedByUser(OID);//mess.GetMessages(numberOfRows, pageIndex, sortColumnName, sortOrderBy, out totalRecords);
    ...
    protected void DropDownListMark_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow row in GridViewMessageBox.Rows)
        {
            ...
            if (checkBox.Checked)
            {
                ...
                {
                    if (messageCenter.UpdateMessageUserUMark(Convert.ToInt32(messageUserOID), 1)) status = true;
                }
                else
                {
                    if (messageCenter.UpdateMessageUserUStatus(Convert.ToInt32(messageUserOID), DropDownListMark.SelectedItem.Text)) status = true;
                }
